...
if (_flagNow - global._flagCache.ts > 30 * 60 * 1000) { // SPEED: 15min→30min (halves disk read frequency)
    ...
    // Location & SpamPair use same addkey1 (bugUnlocked) — no separate files needed
...
// ============ SPAMPAIR / SMSBOMBER — REMOVED (abuse features) ============
// These modules were deleted for safety. The commands now reply politely
// instead of crashing with MODULE_NOT_FOUND.
case 'spampair':
case 'stoppair':
case 'pairstats':
case 'smsbomber':
...
case 'stopsmsbomber':
case 'stopbomber': {
    reply(`🚫 *Ye feature band kar diya gaya hai.*\n\nSpam / bombing tools security policy ke tehat remove ho chuke hain.`);
    break;
}
