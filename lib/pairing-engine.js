 ...
 */
/**
 * Everything that must happen the instant WhatsApp accepts the link:
 *  - credentials mirrored to the database (so any dyno can boot the bot),
 *  - `connected.flag` on disk (same-dyno status endpoint),
 *  - linked_numbers row + bot_sessions=active (dashboard shows the number).
 */
async function completeRegistration(clean, dir) {
  const persisted = await persistAuth(clean, dir);
  if (!persisted) {
    throw new Error('Registered WhatsApp credentials could not be saved');
  }

  try {
    ensureDir(dir);
    fs.writeFileSync(
      path.join(dir, 'connected.flag'),
      JSON.stringify({ number: clean, ts: new Date().toISOString() }),
      'utf8',
    );
  } catch (_) {}

  try {
    await db().finalizePairedNumber(clean);
  } catch (err) {
    console.error('[pairing] finalize failed:', err.message);
    await db().markPairingConnected(clean).catch(() => {});
  }
}
...
async function runPairing(number, options = {}) {
  ...
  if (await hasRegisteredSession(clean)) {
    ...
    await completeRegistration(clean, dir);
  ...
  return new Promise((resolve) => {
    ...
    try {
      sock = makeWASocket({
        ...
        // Keep pairing and normal reconnects on one stable companion profile.
        // This product intentionally identifies its linked device as Safari.
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (status === DisconnectReason.restartRequired || status === 515) {
        // creds.update and connection.update are separate event streams. Flush
        // the accepted registration before closing this socket; otherwise the
        // next socket can reopen with pre-registration credentials and appear
        // as LOGGED_OUT immediately after the phone accepted the code.
